 ...
 *   My B-PLus Tree.c
 *  Created by wangss on 2022.01.17
 *  Last modified: 
 *
...
node *make_node(void)
{
    ...
    new_node->keys = malloc((order - 1) * sizeof(int)); // （阶数-1） 个key
    if (new_node->keys == NULL)
    {
        perror("New node keys array.");
        exit(EXIT_FAILURE);
    }
    new_node->pointers = malloc(order * sizeof(void *)); // （阶数） 个key
...
int testBPTree()
{
    ...
    char *input_file = "../data/testbptree.txt";
    FILE *fp;
    node *root;
